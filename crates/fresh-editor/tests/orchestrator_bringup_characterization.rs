...
// ---------------------------------------------------------------------------
// Branch D: v2 with a session only for an unrelated project. The pick
// finds nothing for the cwd and boots a clean base (issue #2026 — no
// cross-project bleed).
//
// The real fixture (generated by launching in the other project) has
// that project's BASE window at `id == 1`. The clean-base fallback also
// uses id 1, so the unrelated window is SHADOWED by the fresh base and
// is not rebuilt as an inactive shell — only the cwd base survives.
// This is faithful current behavior, surfaced by using a real artifact.
// ---------------------------------------------------------------------------
#[test]
fn v2_cross_project_only_boots_clean_base_at_cwd() {
    ...
    assert_eq!(
        ...
        1,
        "the id-1 other-project window collides with the clean-base fallback and is dropped"
    );
    assert!(
        !window_roots(&editor).contains(&s.other_canon),
        "the unrelated project's window is not surfaced into this launch"
    );
}
